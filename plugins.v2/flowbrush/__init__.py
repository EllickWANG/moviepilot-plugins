...
class FlowBrush(_PluginBase):
    # region 全局定义
    # Based on BrushFlow 4.3.5. Original authors: jxxghp, InfinityPacer, Seed680.

    # 插件名称
    plugin_name = "刷流"
    # 插件描述
    plugin_desc = "自动托管站点刷流。"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "flowbrush_"
    # 加载顺序
    plugin_order = 21
    # 可使用的用户级别
    auth_level = 2
    ...
    def __format_torrent_created_time(value: Any) -> str:
        ...
        if "." in text:
            text = text.split(".", 1)[0]
        try:
            return datetime.strptime(text[:19], "%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%d %H:%M")
        except Exception:
    ...
    def __brush_site_torrents(self, siteid, torrent_tasks: Dict[str, dict], statistic_info: Dict[str, int],
        ...
        for torrent in torrents:
            ...
            self.eventmanager.send_event(etype=EventType.PluginTriggered, data={
                ...
                "event_name": "flowbrush_download_added",
                "hash": hash_string,
                "data": torrent_task,
