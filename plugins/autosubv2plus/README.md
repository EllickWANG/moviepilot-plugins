...
此版本用于私人插件市场维护，适配 MoviePilot 线上环境的 OpenAI/httpx 代理参数。
...
- 支持监听媒体入库事件自动执行字幕生成
- 支持手动触发字幕生成任务
- 支持定时扫描媒体路径，新视频自动加入队列
- 支持任务队列和最多 3 个并行任务
- 支持限制单任务 CPU 线程，降低 ffmpeg / whisper 占用
- 默认使用轻量元数据校验视频，避免完整解码扫描长期占满 CPU
- 支持任务状态列表展示（等待中 / 进行中 / 等待文件完整 / 已完成 / 失败）
- 支持仪表盘 5 秒自动刷新查看任务进度
...
| 定时扫描媒体路径 | 按 cron 周期扫描媒体路径，新视频自动入队 | 是      |
| 扫描周期(cron) | 定时扫描媒体路径的周期              | `*/10 * * * *` |
| 不完整重试(分钟) | 视频不完整时等待后重新检查            | 10      |
| 发送通知     | 是否发送任务执行通知             | 否      |
| 并行任务数    | 同时处理的字幕任务数量，范围 1-3      | 1       |
| CPU线程上限    | 单任务 ffmpeg / whisper 使用的 CPU 线程上限，建议 1-2 | 2 |
| 完整解码校验    | 是否使用 ffmpeg 完整解码扫描视频。高 CPU，默认关闭 | 否 |
...
| 配置项  | 说明                    | 默认值 |
|------|-----------------------|-----|
| 媒体路径 | 手动执行和定时扫描共用的媒体文件或文件夹绝对路径，每行一个 | 空   |
...
7. 翻译后的中文字幕会打上“机翻”标签。
8. 定时扫描不会重新检查已完成、已忽略、处理中或已在队列中的任务；不完整视频会进入等待状态，到期后重新检查。
9. 插件运行时会启动后台线程用于消费任务队列，插件关闭时会中断当前 ffmpeg 子进程，未完成任务会等待下次启动后重新排队。
