...
这是一个 MoviePilot 站点诊断与适配工具箱。它可以选择已有站点测试 RSS 订阅是否可以正常获取和解析，也整合了原 `siteadapter` 的站点索引和用户数据解析适配能力，并支持预览和清理下载器中的缺失文件种子、下载目录中的无种文件。
...
- 可预览本地下载目录中不受当前下载器种子保护的无种文件，并在清理前重新复查下载器路径。
- 详情页采用概览、操作区、主表、折叠详情的工作台布局。
- 缺失文件种子结果默认收进折叠区，可配置 cron 定时自动刷新预览快照，也可单独开启定时自动清理。
- 页面按钮触发的 RSS 测试、RSS 修复、用户数据检查、缺失种子预览和清理、无种文件预览和清理均在后台运行，详情页显示最近结果时间和后台任务状态。
- 记录系统 ERROR/CRITICAL 级别报错，详情页折叠展示，保留时间可选 1 天、3 天或 7 天。
...
| 定时扫描 | 关闭 | 按 cron 周期自动刷新缺失种子预览快照。 |
| 定时自动清理 | 关闭 | 开启后，定时扫描会按最新扫描结果自动清理仍为 `missingFiles` 的种子。是否删除数据文件取决于“同时删除数据文件”。 |
| 扫描周期(cron) | `0 */6 * * *` | 缺失种子定时扫描周期。 |
| 无种文件保护下载器 | 空 | 选择用于建立保护路径的下载器。留空时沿用缺失种子清理下载器。 |
| 无种文件扫描目录 | 空 | 选择本地下载目录。留空时扫描全部本地下载目录。 |
| 无种文件最小未修改时间 | 24 小时 | 只把超过该时长未修改的文件列为候选。 |
| 无种文件排除规则 | 内置临时文件和回收站规则 | 每行一个 fnmatch 规则，可匹配文件名或相对路径。 |
| 错误保留时间 | 3 天 | 系统错误记录保留 1 天、3 天或 7 天。 |
...
“同时删除数据文件”只影响清理动作本身；仅打开这个开关不会自动触发清理。需要点击详情页“清理”，或同时开启“定时自动清理”。

预览无种文件：

```http
POST /api/v1/plugin/sitetoolbox/cleanup/orphan/preview
```

清理最近一次预览中的无种文件：

```http
POST /api/v1/plugin/sitetoolbox/cleanup/orphan
```

无种文件清理只处理本地下载目录内的普通文件。执行清理前会重新读取已选下载器的种子内容路径，并复查文件仍在扫描目录内、未命中排除规则、修改时间达到阈值且不受任何种子路径保护。
