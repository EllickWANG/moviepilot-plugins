...
插件会接管单个订阅的手动搜索接口 `/api/v1/subscribe/search/{id}`。

MoviePilot 原接口通过调度器触发 `subscribe_search`，当定时订阅搜索已经运行时，手动触发会被调度器忽略但接口仍返回成功。插件改为把指定订阅直接交给 `SubscribeChain.search(sid=...)` 后台执行，由订阅链自身的锁排队，避免手动按钮被正在运行的全量订阅搜索吞掉。
